...
def run_binary():
    ...
    reload(c)
    for i in x:
        Q.append(c.cubatic_order(VW[i:i+5],Z[i:i+5],L)[0])
        Q2.append(c.cubatic_order(VW[i:i+5],Z[i:i+5],L)[1])
    print Q
    print Q2
    ...
    #plt.close()
    #rotations(M,VW,L,'Z')
    #cube_connected_face(M, L, var['ndna'], VW.shape[1], n_finish = last, delta = delta)
    #cube_connected_single_face(M, L, var['ndna'], VW.shape[1], n_finish = last,
    #        delta = delta)
    #plt.close()
    #find_networks(M, VW, L, var, last,delta = delta)
    #plt.close()
    #find_lifetime(M,L,x,temp,step_range=30,delta=4)
    #msd(VW,L)
    #plt.close()
    #mylog()
    #plt.close()
...
def run_simple():
    ...
    for i in x:
        #print "finding s(q)"
        structure_factor(V, L, n_start=i,save='sf'+str(i))
        plt.close()
        print "f inding g(s)"
        print V.shape
