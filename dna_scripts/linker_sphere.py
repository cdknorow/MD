...
def find_linker_connections(M, L, var,n_finish, n_start=1,
    ...
    one,two,l1,l2 =  con.both_connections_polymer(connections_A,var['nsphere']*var['ndna'])
    connections_A = con.num_connections(connections_A,var['nsphere'])/2
    steps = range(len(connections_A))
    pyplot.plot(steps, connections_A,
            xlabel='time', ylabel='hyrbid. density',
            save='connections')

    #three = [CC.shape[1] for i in range(len(one))]
    three = [-1 for i in range(len(one))]
    #print 'linkers'
    #print CC.shape[1]
    three = np.subtract(three, one)
    three = np.subtract(three, two)
    print 'number of connections: zero, one, two'
    print three
    print one
    print two
    pyplot.plot3(steps, one, steps, two, steps, three,
            ...
            showleg=True)
    plt.close()
    return steps, connections_A, one, two
...
def find_networks(M, VW, L, var, n_finish, n_start=0, delta=30, rcut=1.0):
    ...
    networks, num_networks = graph.grapher(connections_A,
            N_nodes = VW.shape[1], N_linker = var['Nlinker'], vertex_points = ndna)
...
def find_lifetime(M,L,n_finish=30,n_start=1,delta=4,rcut=1.0,step=5e4):
    ...
    print remain
    pyplot.plot(steps,remain,xlabel='Time',
            ylabel='remaining connections', save='lifetime')
    return steps,remain
def find_polymer_distance(M, L, var, frame = -1,rcut=1.0, step=5e4):
    #The total number of frames we are going to look at
    #Find he number of connections at specific points x
    import MD.analysis.connections as con
    reload(con)
    ...
    bins=13
    for i in l1[-1]:
        edge.append(points.dist(G[frame][i],G[frame][i+1],L)[0])
    hist_edge,xedge,max_edge=histogram(edge,bins)
    plt.close()
    pyplot.plot_bar(xedge,hist_edge,save='linker_1con_distance')
    plt.close()
    edge=[]
    for i in l2[-1]:
        edge.append(points.dist(G[frame][i+1],G[frame][i+1],L)[0])
    hist_edge,xedge,max_edge=histogram(edge,bins)
    pyplot.plot_bar(xedge,hist_edge,save='linker_2con_distance')
    plt.close()
    #get a list of linkers without any connections
...
def run_quick():
    ...
    #end_end(M,L,var)
    #plt.close()
    find_linker_connections(M,L,var,n_finish=last)
    plt.close()
...
def run_debug():
    ...
    #plt.close()
    #print 'sf'
    #structure_factor(V, L, var, n_start=last-20)
    #plt.close()
    #print 'gs'
    #distance_distribution(V,V,L,n_start=last-20)
    #plt.close()
    ...
    #solid_particles(VW,L,var)
    #print 'fl'
    #find_lifetime(M,L,n_finish=last,n_start=last-45)
