...
def int_scatter( np.ndarray  A, np.ndarray  L, np.ndarray  q, np.ndarray  x, int step=1, int skip=1):
    ...
    cdef np.ndarray F = np.zeros((x_len), dtype = DTYPE)
    cdef np.ndarray r = np.zeros((3), dtype = DTYPE)
