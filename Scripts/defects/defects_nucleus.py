...
def find_defects_mod(CV,CW,VW,V,W,L,n_finish=1,n_start=0,delta=20,filters=0.2):
    ...
    print 'interstitials'
    print intersticial
    util.pickle_dump(DV,'DV_n.pkl')
    util.pickle_dump(DW,'DW_n.pkl')

    util.pickle_dump([substitutions, vacancies, intersticial, x],'plot_def_n.pkl')
    pyplot.plot3(x, substitutions, x, particle_frame, x, intersticial,
            ...
            save='defects_per_frame_nucleus',showleg=True)
...
if __name__ == '__main__':
    ...
    print 'MD.L is '
    #M=readxyz.ReadCord(trajectory = 'bcccrystals.xyz',frames=2000)
    #last = 2000
    ...
    MD.last = last
    try:
        MD.V = util.pickle_load('Vn.pkl')
        MD.W = util.pickle_load('Wn.pkl')
        MD.VW = util.pickle_load('VWn.pkl')
    except:
        MD.V = M.cord_auto(['V'])
        MD.W = M.cord_auto(['W'])
        MD.VW = M.cord_auto(['V','W'])
        util.pickle_dump(MD.V,'Vn.pkl')
        util.pickle_dump(MD.W,'Wn.pkl')
        util.pickle_dump(MD.VW,'VWn.pkl')
    CV = util.pickle_load('CV.pkl')
    CW = util.pickle_load('CW.pkl')
    try:
        print 'attempting to load DW, DV'
        DV = util.pickle_load('DV_n.pkl')
        DW = util.pickle_load('DW_n.pkl')
        plot_def = util.pickle_load('plot_def_n.pkl')
    except:
        print 'finding the defects'
        DV, DW, plot_def =  find_defects_mod(CW,CV,MD.VW,MD.V,MD.W,MD.L,n_finish=MD.last-1,n_start=0,delta=25)
