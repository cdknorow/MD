...
def polymer_end_2_end(M, L, frames ):
    ...
    print polymers
    spacers = S.shape[1]/(V.shape[1]*polymers)
    ...
    for count, k in enumerate(frames):
        ...
        c2sp = [np.zeros(V.shape[1]*polymers) for i in range(spacers)]
        for i in range(V.shape[1]):
            for j in range(0,2*polymers,2):
                ...
                for w in range(spacers):
                    c2sp[w][counter] = points.dist(V[count][i],S[count][i*(polymers*spacers)+j/2*spacers+w],L[k])[0]
                counter+=1
        ...
        dset.attrs.create("n_frames",delta)
        for w in range(spacers):
            dset = hpy.create_dataset("%i_%ic2sp"%(k,w), data = c2sp[w])
            dset.attrs.create("N_atoms",V.shape[1]*polymers)
            dset.attrs.create("L",L[k][0])
            dset.attrs.create("n_frames",delta)
    hpy.close()
